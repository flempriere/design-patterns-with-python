...
class UIBuilder:
    ...
    def build_org_chart(self):
        ...
        marketing_vp = employees.ManagingPosition(
            "Vice President (Marketing)", salary=decimal.Decimal(100_000), parent=ceo
        )

        sales_mgr = employees.ManagingPosition(
            "Manager (Sales)", salary=decimal.Decimal(50_000), parent=marketing_vp
        )
        marketing_mgr = employees.ManagingPosition(
            name="Manager (Marketing)",
            salary=decimal.Decimal(50_000),
            parent=marketing_vp,
        )
        ...
        marketing_mgr.add_direct_report(
            employees.JobPosition(
                "Secy", salary=decimal.Decimal(20_000), parent=marketing_mgr
            )
        )

        # create production hierarchy
        production_vp = employees.ManagingPosition(
            name="Vice President (Production)",
            salary=decimal.Decimal(100_0000),
            parent=ceo,
        )
        ...
        production_mgr = employees.ManagingPosition(
            "Manager (Production)", salary=decimal.Decimal(40_000), parent=production_vp
        )
        shipping_mgr = employees.ManagingPosition(
            "Manager (Shipping)", salary=decimal.Decimal(35_000), parent=production_vp
        )
