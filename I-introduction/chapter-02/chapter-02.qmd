...
- The code below creates a simple window with three buttons which can each call a different message box
  ...
  - `showerror` styles the message as an error
    ...
    ![An error box, on Ubuntu this contains a red cross symbol](Examples/03-messagebox/error_box.png)
...
- The message box also provides dialogs for standard prompts to the user
  ...
  - `askquestion` for asking a question
    ...
    ![The `askquestion` dialog looks shows a question mark with *yes* and *no* buttons](Examples/04-questioning-messagebox/question_box.png)

  - `askyesno` for asking a yes or no question

    ![The `askyesno` dialog looks identical to the `askquestion` dialog on Ubuntu](Examples/04-questioning-messagebox/yes_no_box.png)

  - `askyesnocancel` for asking a yes or no question with the option to cancel

    ![The `askyesnocancel` dialog looks like the `askyesno` but has an additional *cancel* button](Examples/04-questioning-messagebox/yes_no_cancel_box.png)

  - `askokcancel` for asking for approval

    ![The `askokcancel` shows a question mark with *ok* and *cancel* buttons](Examples/04-questioning-messagebox/ok_box.png)

  - `askretrycancel` for asking to retry an operation

    ![The `askretrycancel` looks like `askokcancel` but has a warning exclamation mark rather than a question mark](Examples/04-questioning-messagebox/retry_box.png)
...
- Below is an example of a file dialog
  ...
  ![The dialog opened by `filedialog.askopenfilename`. A menu is shown to select files from the specified directory along with an *ok* and *cancel* button. A dropdown menu can be used to narrow the choice of file types](Examples/05-file-dialog/file_dialog.png)
...
- Below is a simple example showing how to put together a simple dialog using multiple components laid out with `pack`
  ...
  ![A simple two-row example using the packed layout. Each row consists of a `tk.Label` left-aligned on the left, and a `tk.Entry` on the right stretched to fill the remaining space](Examples/06-pack/packed_layout.png)
...
- The resulting window looks like
  ...
  ![The basic two-button widget updated to use the `ttk` buttons](Examples/07-ttk-button/ttk_button.png)
